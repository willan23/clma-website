...
import { PaintBucket, Hammer, Building2, CheckCircle2, ChevronRight, Sparkles, Layers, Shield } from "lucide-react";
...
export function ServicesSection() {
  ...
  return (
    <section id="servicos" className="py-28 relative overflow-hidden bg-[#08121E]">
      <div className="container relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20 space-y-4">
          ...
          <h2 className="text-4xl sm:text-6xl font-black text-white tracking-tight">
            Especialidades de <span className="text-amber-400">Excelência</span>
          </h2>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {services.map((svc, idx) => (
            <motion.div
              ...
              className="glass-card glass-card-hover p-8 sm:p-10 rounded-[2.5rem] flex flex-col justify-between relative group overflow-hidden border border-white/10"
            >
              ...
              <div>
                {/* Header Icon */}
                <div className={`w-16 h-16 rounded-2xl ${svc.iconBg} border ${svc.borderColor} flex items-center justify-center mb-8 shadow-xl group-hover:scale-110 transition-transform duration-500`}>
                ...
                <p className="text-slate-300 text-sm mb-8 leading-relaxed font-normal">
                  {svc.desc}
                </p>
                ...
                <div className="space-y-4 mb-8">
                  {svc.details.map((item, i) => (
                    <div key={i} className="p-4 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                      <p className="text-xs font-black text-amber-400 uppercase tracking-wider mb-1 flex items-center gap-2">
                        <CheckCircle2 className="w-3.5 h-3.5 text-amber-400" />
                        {item.t}
                      </p>
                      <p className="text-[11px] text-slate-300 leading-relaxed font-normal">{item.d}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <button className="w-full mt-auto py-3.5 px-5 rounded-xl glass-card hover:bg-amber-500 text-white hover:text-slate-950 font-black text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all group-hover:border-amber-400/50">
                    <span>Especificações Técnicas</span>
                    <ChevronRight className="w-4 h-4" />
                  </button>
                </DialogTrigger>
                <DialogContent className="bg-[#0F2338] text-white border-amber-400/30 max-w-2xl rounded-3xl p-8">
                  <DialogHeader>
                    <DialogTitle className="text-2xl font-black text-amber-400 flex items-center gap-3">
                    ...
                    <div className="grid gap-4">
                      {svc.details.map((d, i) => (
                        <div key={i} className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-1">
                          <h4 className="font-bold text-white text-sm">{d.t}</h4>
                          <p className="text-xs text-slate-300">{d.d}</p>
                        </div>
                      ))}
                    </div>
